...
const Jobs = () => {
  ...
  const firstSix = jobs.slice(0, GRID_LIMIT);
  const jobsToShow = showMore ? jobs : firstSix;
  ...
  return (
    <StyledJobsSection>
      <h2 ref={revealTitle}>Other Noteworthy Jobs</h2>
      ...
      <ul className="jobs-grid">
        {prefersReducedMotion ? (
          <>
            {jobsToShow &&
              jobsToShow.map(({ node }, i) => (
                <StyledJobs key={i}>{jobInner(node)}</StyledJobs>
              ))}
          </>
        ) : (
            ...
            {jobsToShow &&
              jobsToShow.map(({ node }, i) => (
                <CSSTransition
                  ...
                  <StyledJobs
                    key={i}
                    ref={el => (revealJobs.current[i] = el)}
                    style={{
